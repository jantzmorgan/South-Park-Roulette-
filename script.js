	...
	favorites.forEach((ep, index) => {
      ...
      <div class="stars">
        ...
        `).join("")}
			...
			<button class="goat-btn ${ep.goat ? "goat-active" : ""}" onclick="toggleGoat(${index})">
  GOAT
</button>
  ...
  <button class="remove-btn" onclick="removeFavorite(${index})">
    ...
    `;
		...
		openBtn.addEventListener("click", () => {
			...
			window.location.href = ep.driveUrl.trim();
		});
...
function sortFavoritesByRating() {
	favorites.sort((a, b) => {
		if ((b.goat || false) !== (a.goat || false)) {
			return (b.goat || false) - (a.goat || false);
		}
